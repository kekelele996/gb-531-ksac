...
| 安全复核员 `safety_reviewer` | `reviewer` | `reviewer123` | 复核偏差、确认/作废评估、维护生命周期与登记/撤销保护层计划停用 |
...
- 保护层停用台账：安全复核员登记计划检修的停用原因与起止时间；同一措施后提交的重叠时段（端点相接除外）返回 `409` 不生效。停用窗口为待停用/停用中/已结束三态，窗口自然到期或提前撤销后无需人工恢复，措施自动重新纳入；每次登记与撤销都写入审计证据。
- 覆盖推演：冻结输入，构建原因到后果路径，找出未保护路径，按独立性键去重并保存评分步骤、输入哈希与算法版本。冻结时刻落在停用窗口内的措施会连同窗口一起写入不可变快照，但不参与独立性去重和覆盖分。
...
4. 只有生命周期有效且 `last_verified_at + test_interval_days` 未过期的保护层参与计算；冻结时刻处于计划停用窗口内的保护层（`safeguard_outages`，未撤销且 `starts_at <= 冻结时刻 < ends_at`）连同停用原因一起写入快照，但在独立性分组之前剔除：它既不占据独立性键，也不贡献覆盖分，并在评分步骤中以 `eligibility-filter` 说明。
5. 停用窗口结束（自然到期或提前撤销）后无需任何人工操作，后续运行重新纳入该措施；历史评估仍使用各自的冻结快照。
6. 每层按 `effectiveness` 贡献覆盖度。
7. 保存覆盖分、评估前后风险、未覆盖路径、评分步骤、算法版本和不可变输入快照。重放使用原始快照，因此相同输入产生相同结果。
...
`SafeguardOutageState = pending | active | ended`（待停用 / 停用中 / 已结束）

- 后端定义与推导：`backend/internal/constants/safeguard_outage_state.go`（状态由撤销标记与窗口起止时间相对当前时刻推导，不持久化状态字段）
- 数据模型：`backend/internal/model/safeguard_outage.go`；DTO 与仓储：`dto/safeguard_outage.go`、`repository/safeguard_outage_repository.go`
- 业务与测试：`service/safeguard_outage_service.go`、`service/safeguard_outage_service_test.go`
- 前端定义：`frontend/src/types/enums/safeguard-outage-state.ts`
- 前端消费：`types/safeguard-outage.ts`、`api/safeguard-outage.ts`、`stores/safeguard-outage.ts`、`pages/SafeguardOutagesPage.vue`、`pages/SafeguardsPage.vue`、`pages/CoveragePage.vue`
...
| `/safeguard-outages` | `SafeguardOutage + Safeguard` | 登记/撤销计划检修停用、三态台账、覆盖目标筛选 |
| `/coverage` | `CoverageEvaluation + DeviationScenario + Safeguard + SafeguardOutage` | 幂等运行、轮询、路径解释、版本对比、确认/作废 |
...
| `GET/POST` | `/api/v1/safeguard-outages` / `/safeguard-outages/safeguards/:safeguardId` | 停用台账列表（`status=active/pending/ended`、`safeguard_id`、`scenario_id`）与登记停用窗口（安全复核员/管理员） |
| `GET` | `/api/v1/safeguard-outages/:id` | 停用登记详情 |
| `POST` | `/api/v1/safeguard-outages/:id/revoke` | 提前撤销停用（窗口立即结束，措施自动重新纳入） |
...
- 覆盖分低于预期：检查过期保护层、失效生命周期和重复 `independence_key`，再打开评分解释查看去重原因；若保护层处于计划停用窗口，评分步骤会以 `eligibility-filter` 标明停用起止与原因，窗口结束后自动恢复计入。
- 停用登记返回 `409`：与该措施已登记且未撤销的窗口重叠；调整起止时间后重试，端点相接（前窗结束时刻即新窗开始时刻）允许登记。
