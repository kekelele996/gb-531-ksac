...
import { Ban, CalendarClock, FileCheck2, Pencil, Plus, RefreshCw, RotateCcw, ShieldCheck } from 'lucide-vue-next'
...
import { outageStatusLabels, type SafeguardOutage, type SafeguardOutageInput } from '../types/safeguard-outage'
...
const outageDialog = ref(false)
const outageSaving = ref(false)
const outageTarget = ref<Safeguard>()
const outageForm = reactive<SafeguardOutageInput>({ reason: '', starts_at: '', ends_at: '' })
...
const visibleOutages = computed(() => filterScenario.value ? outages.items.filter((x) => x.scenario_id === filterScenario.value) : outages.items)
...
const fmtTime = (value: string) => new Date(value).toLocaleString('zh-CN', { hour12: false })
const outageBySafeguard = computed(() => {
  const map = new Map<number, SafeguardOutage>()
  for (const item of outages.items) {
    if (item.status !== 'active' && item.status !== 'scheduled') continue
    const existing = map.get(item.safeguard_id)
    if (!existing || (existing.status !== 'active' && item.status === 'active')) map.set(item.safeguard_id, item)
  }
  return map
})
const outageOf = (id: number) => outageBySafeguard.value.get(id)
...
async function refresh() { try { await Promise.all([store.load(), scenarios.load(), outages.load()]) } catch (error) { ElMessage.error(errorMessage(error)) } }
...
function openOutage(item: Safeguard) { outageTarget.value = item; const start = new Date(); Object.assign(outageForm, { reason: '', starts_at: start.toISOString(), ends_at: new Date(start.getTime() + 4 * 3600 * 1000).toISOString() }); outageDialog.value = true }
...
async function saveOutage() {
  if (!outageTarget.value) return
  outageSaving.value = true
  try {
    await outages.register(outageTarget.value.id, { reason: outageForm.reason, starts_at: new Date(outageForm.starts_at).toISOString(), ends_at: new Date(outageForm.ends_at).toISOString() })
    outageDialog.value = false
    ElMessage.success('停用时段已登记；窗口内冻结的评估将自动剔除该措施')
  } catch (error) { ElMessage.error(errorMessage(error)) } finally { outageSaving.value = false }
}
async function revokeOutage(item: SafeguardOutage) {
  try {
    await ElMessageBox.confirm(`撤销「${item.safeguard_name}」${fmtTime(item.starts_at)} 起的停用登记？撤销后该措施立即恢复参与评估。`, '确认撤销', { type: 'warning' })
    await outages.revoke(item.id, '安全复核员撤销停用登记')
...
<template>
  <AppShell>
    <PageHeader eyebrow="INDEPENDENT PROTECTION REGISTER" title="保护层台账" description="核对覆盖目标、独立性键、有效性与验证期限；同一路径的重复独立性键只计一次。计划检修停用在窗口内自动剔除，结束后自动恢复计分。">
    ...
    <section class="data-section">
      <el-table v-loading="store.loading" :data="visible" row-key="id" empty-text="暂无保护措施">
        ...
        <el-table-column label="计划停用" min-width="150"><template #default="{ row }"><template v-if="outageOf(row.id)"><span class="state-label" :class="`outage-${outageOf(row.id)?.status}`">{{ outageStatusLabels[outageOf(row.id)!.status] }}</span><small class="cell-note">{{ outageOf(row.id)?.status === 'active' ? `至 ${fmtTime(outageOf(row.id)!.ends_at)}` : `起 ${fmtTime(outageOf(row.id)!.starts_at)}` }}</small></template><small v-else class="cell-note">无停用窗口</small></template></el-table-column>
        <el-table-column label="操作" width="240" fixed="right"><template #default="{ row }"><el-tooltip content="查看证据"><el-button circle text aria-label="查看证据" @click="showEvidence(row)"><FileCheck2 :size="16" /></el-button></el-tooltip><el-tooltip v-if="canEdit" content="编辑"><el-button circle text aria-label="编辑" @click="openEdit(row)"><Pencil :size="16" /></el-button></el-tooltip><el-tooltip v-if="canReview" content="记录本次验证"><el-button circle text type="success" aria-label="验证保护层" @click="action(row, 'verify')"><ShieldCheck :size="16" /></el-button></el-tooltip><el-tooltip v-if="canReview" content="登记计划检修停用"><el-button circle text type="warning" aria-label="登记停用" @click="openOutage(row)"><CalendarClock :size="16" /></el-button></el-tooltip><el-tooltip v-if="canReview && ['pending','active','expired'].includes(row.lifecycle_state)" content="标记失效"><el-button circle text type="danger" aria-label="标记失效" @click="action(row, 'invalidate')"><Ban :size="16" /></el-button></el-tooltip><el-tooltip v-if="canReview && row.lifecycle_state === 'invalid'" content="恢复到待验证状态"><el-button circle text type="success" aria-label="恢复" @click="action(row, 'restore')"><RotateCcw :size="16" /></el-button></el-tooltip></template></el-table-column>
      </el-table>
    </section>
    <section class="data-section">
      <div class="section-heading"><div><p class="eyebrow">PLANNED OUTAGE LEDGER</p><h2>计划停用台账</h2></div><span>{{ visibleOutages.length }} 条登记 · 窗口结束后自动恢复，无需人工操作</span></div>
      <el-table v-loading="outages.loading" :data="visibleOutages" row-key="id" empty-text="暂无停用登记">
        <el-table-column label="保护措施" min-width="190"><template #default="{ row }"><div class="primary-cell"><strong>{{ row.safeguard_name }}</strong><span>{{ scenarioLabel(row.scenario_id) }}</span></div></template></el-table-column>
        <el-table-column label="停用原因" min-width="200"><template #default="{ row }">{{ row.reason }}</template></el-table-column>
        <el-table-column label="停用窗口" min-width="210"><template #default="{ row }"><span class="date-cell">{{ fmtTime(row.starts_at) }}</span><small class="cell-note">至 {{ fmtTime(row.ends_at) }}</small></template></el-table-column>
        <el-table-column label="状态" width="100"><template #default="{ row }"><span class="state-label" :class="`outage-${row.status}`">{{ outageStatusLabels[row.status as SafeguardOutage['status']] }}</span></template></el-table-column>
        <el-table-column label="登记 / 撤销" min-width="190"><template #default="{ row }"><div class="primary-cell"><span>{{ row.registered_by_name }} · {{ fmtTime(row.created_at) }}</span><small v-if="row.revoked_at" class="cell-note">{{ row.revoked_by_name }} 撤销：{{ row.revoke_reason }}</small></div></template></el-table-column>
        <el-table-column label="操作" width="90" fixed="right"><template #default="{ row }"><el-tooltip v-if="canReview && (row.status === 'scheduled' || row.status === 'active')" content="撤销该停用登记"><el-button circle text type="danger" aria-label="撤销停用登记" @click="revokeOutage(row)"><RotateCcw :size="16" /></el-button></el-tooltip></template></el-table-column>
      </el-table>
    </section>
    ...
    <el-dialog v-model="outageDialog" :title="`登记计划停用 · ${outageTarget?.name ?? ''}`" width="min(560px, 94vw)">
      <el-form label-position="top" @submit.prevent="saveOutage">
        <el-form-item label="停用原因" required><el-input v-model="outageForm.reason" type="textarea" :rows="3" placeholder="例如：计划检修 / 定期试验，注明工单号与影响范围" /></el-form-item>
        <div class="form-grid two"><el-form-item label="停用开始" required><el-date-picker v-model="outageForm.starts_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" /></el-form-item><el-form-item label="停用结束" required><el-date-picker v-model="outageForm.ends_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" /></el-form-item></div>
        <p class="cell-note">与已登记时段重叠的提交不会生效；冻结时刻落在窗口内的评估快照将自动剔除该措施，时段结束后自动恢复。</p>
      </el-form>
      <template #footer><el-button @click="outageDialog = false">取消</el-button><el-button type="primary" :loading="outageSaving" @click="saveOutage">登记停用</el-button></template>
    </el-dialog>
